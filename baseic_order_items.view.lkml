...
view: basic_order_items { # creates a view file with the name 'basic_order_items'
  ...
  #### The Arena Group, a Looker customer, asked for logic to search before and after a particular date for a specificied number of days. ##########
  parameter: selected_date {
    type: date
  }

  parameter: number_of_days_prior {
    description: "number of days preceding the selected_date which you would like to include in the query"
    type: number
  }

  parameter: number_of_days_after {
    label: "number of days following the selected_date which you would like to include in the query"
    description: "number of days following the selected_date which you would like to include in the query"
    type: number
  }

  dimension: first_date {
    description: "this SQL will return the first date in the desired time range from using selected_date and number_of_days_prior"
    hidden: yes
    type: date
    sql: DATE_ADD({% parameter selected_date %}, INTERVAL -{% parameter number_of_days_prior %} DAY) ;;
  }

  dimension: last_date {
    description: "this SQL will return the first date in the desired time range from using selected_date and number_of_days_after"
    hidden: yes
    type: date
    sql: DATE_ADD({% parameter selected_date %}, INTERVAL {% parameter number_of_days_after %} DAY) ;;
  }
### Close the Arena Group code ######################################################################################################
  ...
  measure: total_sale_price {
    ...
    drill_fields: [total_sale_price,average_sale_price, count]
  }
